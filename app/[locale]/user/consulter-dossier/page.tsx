...
import { getDossiers, downloadDossierFile } from "@/lib/dossier-utils"
...
export default function ConsulterDossierPage() {
  ...
  const handleViewDetails = (dossierId: string) => {
    router.push(`/${language}/user/consulter-dossier/${dossierId}`)
  }

  const handleDownload = async (dossierId: string, type: "pv" | "note") => {
    try {
      ...
      link.href = url
      link.setAttribute("download", `${type === "pv" ? "PV" : "Note"}-${dossierId}.pdf`)
      ...
      alert(`${t_local("downloadError")}: ${type === "pv" ? "PV" : "Note"}`)
    }
  }
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            ...
            <div className="relative w-full sm:w-48">
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger
                  className={statusFilter !== "all" ? "border-[#2E8BC0]" : ""}
                >
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        {dossier.status === "ended" ? (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleDownload(dossier.id, "pv")}
                            className="bg-green-600 hover:text-black text-white transition-colors duration-200"
                          >
                        ...
                        {dossier.status === "ended" ? (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleDownload(dossier.id, "note")}
                            className="bg-green-600 hover:text-black text-white transition-colors duration-200"
                          >
                            <Download className="h-4 w-4 mr-1" />
                            {t_local("note")}
